...
int main(){
    ...
    clock_t end = clock();
    //We expect this value to be 300000 (100000 * 3), but it rarely will be!
...
void * addFunction(){
    ...
    for(int i = 0; i < 100000; i++){
        val = val + 1;
        //the increment is written above for a reason:
        //At the assembly level, this will get translated to the following:
        //mov rax, (val); #or other register
        //add rax, 1; #adds 1 to rax (val)
        //mov (val), rax; #stores the updated value back to val
        //Due to scheduling, its possible for the thread to get paused at ANY of these instructions
        //So its possible to load val into rax, thread will be suspended, and then when its restored, the current
        //value inside of val is old and now incorrect, but will be incremented by 1 and stored back!
        local_counter++;
        if(!(local_counter % 10)){
